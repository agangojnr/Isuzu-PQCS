...
public function delayedunits(){
    ...
    $allschdates = Production_target::where('schedule_part','entire')->whereBetween('date', ['2022-01-01', $yesterday])->groupby('date')->get(['date']);
    foreach($allschdates as $schdt){ $allprodndays[] = $schdt->date; }
    $yesterday = end($allprodndays);
    $delays = Unitmovement::where([['datetime_in','!=',$yesterday],['datetime_in','!=',$today],['current_shop','>',0]])->get();
